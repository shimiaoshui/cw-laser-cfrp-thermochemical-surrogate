...
class CFG:
    ...
    train_pairs: list = field(default_factory=lambda: [(1.0,100.0),(5.0,100.0),(10.503,100.0),(15.0,75.0),(20.0,40.0)])
    ...
    # data
    java_path: str = r"./最终烧蚀2_nosolve20W.java"
...
@torch.no_grad()
def eval_dataset(cfg: CFG, model: AblationSIRENFiLM, ds: Dataset, const: Dict[str, torch.Tensor], scales: Dict[str,float],
...
def discover_datasets(cfg: CFG) -> List[DatasetSpec]:
    ...
    for fp in candidates:
        ...
        m_p = re.search(r"(?P<p>[0-9.]+)W", bn)
        Pnom = float(m_p.group("p")) if m_p else float(getattr(cfg, "default_power", 0.0))
    ...
    for (folder, dur, Pnom), tps in sorted(groups.items(), key=lambda kv: (kv[0][1], kv[0][2], kv[0][0])):
        ...
        spec = DatasetSpec(
            name=f"{dur:g}s_{Pnom:g}W" if m_p else f"{dur:g}s_base",
...
def train(cfg: CFG):
    ...
    device = cfg.device
    model = AblationSIRENFiLM(cfg).to(device)
